...
export function heygenAuthHeaders() {
  ...
  if (cred?.expired)
    throw new Error(
      "HeyGen OAuth token expired — run `bunx hyperframes auth refresh` (or `bunx hyperframes auth login`)",
    );
  throw new Error(
    "no HeyGen credentials — set $HEYGEN_API_KEY, or run `bunx hyperframes auth login` (writes ~/.heygen/credentials)",
  );
}
