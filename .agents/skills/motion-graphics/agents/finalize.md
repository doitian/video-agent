...
1. **Snapshots** — run `bunx hyperframes snapshot --at <proof-times>` and inspect overflow, off-canvas content, text collisions, empty frames, wrong content, and unreadable motion.
...
3. **Recheck** — rerun `bunx hyperframes lint`, `bunx hyperframes check`, and the affected snapshots. Return the result to the orchestrator without rendering.
