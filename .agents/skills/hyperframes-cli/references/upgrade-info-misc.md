...
```bash
bunx hyperframes info                   # project metadata
bunx hyperframes info ./my-video        # specific project
bunx hyperframes info --json
```
...
```bash
bunx hyperframes upgrade                # check + interactive prompt
bunx hyperframes upgrade --check        # check and exit, no prompt (agent-friendly)
bunx hyperframes upgrade --check --json # machine-readable: current / latest / updateAvailable
bunx hyperframes upgrade --yes          # print upgrade commands without prompting
```

Compares the installed CLI version against npm latest.

`--project [dir]` bumps a **project's** pinned scripts instead of the global install: it rewrites every `bunx …hyperframes@<version>…` in `<dir>/package.json` (default cwd) to npm-latest. Always invoke it unpinned (`bunx hyperframes@latest upgrade --project`) — a project scaffolded on an old CLI stays frozen otherwise. `--project . --check` reports the delta without writing; add `--json` for `{ changed, from, to, path }`. Pass the dir explicitly whenever another flag follows `--project` — on older releases a bare `--project` consumes the next flag as its directory value.

## compositions, docs

```bash
bunx hyperframes compositions           # list compositions in project
bunx hyperframes compositions --json
bunx hyperframes docs                   # list available topics
bunx hyperframes docs rendering         # print one topic inline in the terminal
```
...
```bash
bunx hyperframes benchmark              # run the preset matrix in current project
bunx hyperframes benchmark ./my-video   # specific project
bunx hyperframes benchmark --runs 5     # repeat each config N times (default 3)
bunx hyperframes benchmark --json
```
...
```bash
bunx hyperframes telemetry status      # show telemetry state
bunx hyperframes telemetry disable     # disable anonymous usage telemetry
bunx hyperframes telemetry enable      # re-enable telemetry
```
...
```bash
bunx hyperframes tts
bunx hyperframes transcribe
bunx hyperframes remove-background
```
