...
description: >
  ...
  cloud, cloudrun, feedback, lambda, doctor, browser, info, upgrade, compositions, docs,
...
Run commands as `bunx hyperframes ...` unless project instructions provide a wrapper. Obey the wrapper when present. The CLI requires Node.js 22 or newer and FFmpeg.

## Development loop

1. **Scaffold:** `bunx hyperframes init <project>` or capture a site. In non-TTY mode, pass `--non-interactive --example=<name>`.
2. **Find the move:** before authoring motion by hand, search for a primitive that already does it: `bunx hyperframes catalog --query "reveal a headline one line at a time"`. Ask for the effect you want rather than the mechanism you have in mind. Install with `bunx hyperframes add <name>` (see `/hyperframes-registry`). Author by hand only once nothing fits.
3. **Author:** write the composition using `/hyperframes-core`.
4. **Get fast feedback while editing:** run `bunx hyperframes lint` after the first HTML pass and after structural changes.
5. **Run the final gate:** run `bunx hyperframes check`; it reruns lint before opening the browser. Do not prepend a redundant standalone lint invocation. Add `--snapshots` for annotated overview frames and finding crops.
...
7. **Open the final Studio preview:** run `bunx hyperframes preview --background`, verify the URL returns HTTP 200, hand the timeline project URL to the user, and ask whether to revise or render. Keep it alive until review ends.
...
bunx hyperframes preview --background
bunx hyperframes render --quality high --output out.mp4
test -s out.mp4
...
```bash
bunx hyperframes snapshot --at <t1>,<t2>,<t3>
```
...
- `doctor --json` always exits zero. Gate on its payload:
  ...
  ```bash
  bunx hyperframes doctor --json | jq -e '.ok' >/dev/null
  ```
...
```bash
bunx hyperframes preview --context --json --context-fields selection
```
...
| Need                                     | Command                                                                       |
| ---------------------------------------- | ----------------------------------------------------------------------------- |
| Fast local iteration                     | `bunx hyperframes render --quality draft`                                      |
| Final local delivery                     | `bunx hyperframes render --quality high --output out.mp4`                      |
| Reproducible container render            | `bunx hyperframes render --docker --strict --output out.mp4`                   |
| Local variable-driven batch render       | `bunx hyperframes render --batch rows.json --output "renders/{name}.mp4"`      |
| HeyGen-hosted zero-infrastructure render | `bunx hyperframes cloud render`                                                |
| Self-managed distributed AWS render      | `bunx hyperframes lambda render <project> --width 1920 --height 1080 --wait`   |
| Self-managed distributed GCP render      | `bunx hyperframes cloudrun render <project> --width 1920 --height 1080 --wait` |
...
```bash
bunx hyperframes feedback --rating <0-10> --comment "<specific result or friction>"
```
...
```bash
bunx hyperframes present <project-dir> --port 3004 --no-open
bunx hyperframes beats <project-dir> --json
bunx hyperframes keyframes <project-dir> --json
bunx hyperframes media-treatment --capabilities
bunx hyperframes figma asset KEY:10-20
```
